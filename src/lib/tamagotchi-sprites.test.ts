...
describe("tamagotchi sprite descriptors", () => {
  ...
  test("uses manifest-backed mood-aware strips for jurassic wave 1 species after approval", () => {
    const sprite = getTamagotchiSpriteSheet(6, "adult", "happy");

    expect(sprite.expectedSrc).toBe("/tamagotchi/006/adult-happy.png");
    expect(sprite.expectedFrameCount).toBe(4);
    expect(sprite.fallbackFrameCount).toBe(4);
    expect(sprite.frameDurationMs).toBe(170);
  });
